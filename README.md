...
- **결정 관리표(Wayfinder의 Map)** == 결정할 질문, 먼저 답해야 할 질문, 진행 상태와 결론을 모아둔 기록. 실제 그림이 아니라 상위 이슈와 개별 결정 티켓으로 관리
- **결정 티켓** == ‘무엇을 어떻게 할까?’에 답해야 끝나는 질문 카드
...
Wayfinder 원문은 목적지까지 결정할 순서를 찾아간다는 비유로 **Map**이라고 부릅니다. 여기서는 **결정 관리표**라고 풀어 씁니다. 예를 들어 ‘공지 열람 대상을 누구로 할까?’를 먼저 정해야 ‘읽을 때 로그인이 필요한가?’를 정할 수 있습니다. 이 질문들의 연결과 답을 기록하는 것이며, 지도 그림이나 프로그래밍의 Map 자료형을 뜻하지 않습니다.
...
| **그릴링 vs Wayfinder** | 그릴링 = 질문으로 결정하기 / Wayfinder = 여러 세션의 결정 질문·의존 관계·결과 관리. 내부에서도 그릴링 사용 |
| **결정 관리표 vs 구조도** | 결정 관리표 = 질문·선후 관계·결론을 기록 / 구조도 = 시스템이 어떻게 연결돼 있나 |
| **결정 티켓 vs 구현 티켓** | 결정 = 답을 정하면 끝 / 구현 = 만들고 검사해야 끝 |
...
| `wayfinder` | `grill-with-docs` + 필요할 때 `research` + 필요할 때 `prototype` + 결정을 위한 준비 작업 + 안건별 티켓·선후 관계·진행 상태 관리 | 질문·기록·조사·실험을 연결해 큰 기획의 진행 관리 |
...
**폴더 이름과 구조도 이 단계에서 합의합니다.** `backend/`는 서버 코드, `frontend/`는 화면 코드의 위치입니다. 이름 자체에 특별한 기능은 없지만 실행 명령과 CI/CD의 경로가 이를 사용하므로 일치시켜야 합니다. 2강에서는 작업 규칙을 준비하고, 여기서 폴더 구성을 확정한 뒤 구현 단계에서 실제 코드를 만듭니다. 단순한 폴더 이름 선택만으로 ADR을 만들 필요는 없습니다.
...
| --- | --- | --- |
| 공통점 | `grilling`과 `domain-modeling`으로 질문하고 용어·결정을 정리 | 관리표 작성과 결정 논의에서 같은 스킬을 사용 |
...
| 기록의 역할 | `CONTEXT.md`는 용어집, ADR은 필요한 중요한 결정의 이유 | 상위 관리 이슈는 색인, 개별 결정의 상세 답은 자식 티켓에 기록 |

**순서 A — 크기를 아직 모를 때:** `/grill-with-docs` → 여러 세션에 걸친 결정 관리가 필요해지면 `/wayfinder`로 기존 합의와 남은 질문 정리 → 관리표 생성 → 후속 세션마다 결정 티켓 해결.

**순서 B — 처음부터 큰 작업임을 알 때:** `/wayfinder` → 목적지와 범위를 질문으로 합의 → 관리표 생성 → 후속 세션마다 결정 티켓 해결.

두 경로 모두 필요한 결정이 정리된 뒤, 목적지가 구현 명세라면 `/to-spec` → `/to-tickets` → `/implement`로 이어갑니다. 관리표만 작성하고 곧바로 명세로 넘어가는 흐름이 아닙니다. 트래커 설정이 없는 프로젝트는 `/setup-matt-pocock-skills`로 준비합니다. 이미 준비되어 있다면 다시 설정할 필요는 없습니다.
...
**결정 관리표 예시:** ‘조직과 개인의 자료 소유권’ → ‘퇴사·초대 취소 시 접근 정책’, ‘조직별 과금 주체’. 소유권과 기존 데이터 조사 결과가 갖춰지면 ‘기존 프로젝트의 조직 배정 기준’을 결정합니다. 예를 들어 공동 사용 중인 개인 프로젝트가 발견되면 ‘소유 조직을 누가 확정하는가’라는 새 질문을 추가합니다.

**첫 세션과 다음 세션:** 첫 세션은 ‘조직 계정 전환 명세에 필요한 결정을 마친다’는 목적지와 질문 관리표를 만듭니다. 다음 세션은 선행 조건이 해결된 ‘조직과 개인의 자료 소유권’ 티켓 하나를 사람과 논의합니다. ‘모든 테이블에 organization_id 추가’는 이 단계의 결정 티켓이 아니라, 설계가 정해진 뒤 검토할 구현 작업입니다.
...
먼저 목적지와 범위를 나와 확인하고 결정 관리표와 선후관계를 만들어줘.
```
...
**그릴 위드 독스로 충분한 범위:** 기존 연동의 오류 문구를 바꾸거나, 이미 합의된 정책대로 결제 실패 안내를 추가하는 일입니다. 결제 코드가 크다는 이유만으로 관리표가 필요하지 않습니다.
...
**결정 관리표 예시:** ‘기존 승인 건의 취소 지원 조건 조사’ → ‘이전 주문의 환불 경로’. ‘콜백 중복·순서 역전 시 상태 규칙’을 합의한 뒤 작은 상태 모델 실험으로 검토합니다. 두 경로가 정리되면 ‘신규 주문 전환 조건과 실패 시 복구 범위’를 결정합니다. 외부 계약·API 동작은 이 예시가 확정해 주는 사실이 아닙니다.
...
**결정 관리표 예시:** ‘현재 병목과 트랜잭션 범위 확인’ → ‘분리가 필요한가’. 분리가 필요하다고 결정되면 ‘예약·확정·해제의 소유 주체’ → ‘주문 실패·메시지 재전달 시 정합성 규칙’ → ‘기존 예약 데이터 이전 및 전환 조건’으로 질문을 구체화합니다. 질문 자체가 아직 불명확한 영역은 억지로 티켓을 만들지 않고 관리표에 미정 영역으로 남깁니다.

**완료 기준:** 분리 여부와 그 이유, 분리한다면 유지해야 할 동작과 실패 처리·전환 조건이 합의됩니다. 목적지가 ‘분리 여부 결정’이었다면 그 결정에서 관리표가 끝나며, 서비스 구현까지 자동 확장하지 않습니다.

```text
/wayfinder
주문과 재고가 같은 DB 트랜잭션에 있는 모놀리스에서 재고 분리를 검토하고 있어.
아직 분리하기로 확정하지 않았어. 목적지는 분리 여부와 판단 근거를 결정하는 것이야.
현재 코드·관측 가능한 성능 자료를 확인하고 결정에 필요한 질문 관리표를 만들어줘.
분리 불필요라는 결론도 허용하고, 구현 티켓은 아직 만들지 마.
```

**관리표를 이어갈 때:** 생성된 관리표 식별자를 사용해 `/wayfinder [실제 관리표 URL 또는 번호]`로 재개합니다. 별도 티켓을 지정하지 않으면 진행 가능한 미할당 결정 티켓을 선택합니다. 원문은 연구 티켓을 제외하고 세션당 하나만 해결하도록 하며, 사람의 판단이 필요한 질문은 실제 답변을 받아 해결합니다. 답을 티켓에 남기고 닫은 다음, 관리표에는 요약과 링크를 추가하고 새로 드러난 질문을 연결합니다.

**끝나면:** 합의한 목적지까지 필요한 결정이 정리되고, 후속 담당자가 결정 근거와 남은 작업을 찾을 수 있습니다. 기본은 계획이며, 실행까지 포함하려면 관리표 Notes에 별도로 범위를 정해야 합니다.
...
| `wayfinder/SKILL.md`의 Map·Invocation | 여러 세션에 걸친 결정 관리표와 자식 티켓을 트래커에 유지합니다. 관리표 작성과 티켓 해결은 구분하며 연구를 제외하면 세션당 하나의 티켓을 해결합니다. |
