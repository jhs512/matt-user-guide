...
막히면 같은 대화에 다음처럼 적습니다. 비밀번호나 토큰은 붙여 넣지 않습니다.

```text
지금 3강을 진행 중이야.
방금 나온 설명 중 무엇을 선택해야 할지 모르겠어.
내가 지금 해야 하는 행동 한 가지와, 끝났는지 확인할 방법을 알려줘.
파일이나 명령이 필요하면 어느 폴더에서 사용하는지도 적어줘.
```
...
```text
/to-spec
방금 실험에서 선택한 모바일 카드형 목록을 기존 명세에 반영해줘.
CONTEXT.md, 관련 ADR, .scratch/notice-board/spec.md를 확인하고
기존 로그인·작성·수정·삭제 규칙은 유지해줘.
무엇을 확인했고 무엇은 아직 확인하지 못했는지 구분해줘.
```

명세를 확인한 뒤 다음을 **하나씩** 실행합니다.

```text
/to-tickets
방금 명세에서 변경한 목록 화면에 필요한 작업만 티켓으로 작성해줘.
기존에 완료한 기능의 티켓을 새로 만들거나 미완료로 바꾸지 마.
```

```text
/implement
방금 만든 목록 화면 변경 티켓만 구현하고 검사해줘.
실험용 화면 전환 도구는 운영 결과에 포함하지 마.
```
