...
- **사내 장비 대여 도구 신규 개발:** 지금은 운영팀이 스프레드시트로 노트북을 배정합니다. 신청·승인·반납을 웹으로 옮기려면 중복 신청, 승인자, 미반납 상태를 먼저 합의합니다. `/grill-with-docs` → 명세 → 티켓으로 충분히 시작할 수 있고, 빈 저장소라는 이유로 Wayfinder는 필요하지 않습니다.
- **거래처 문의 접수 포털:** 이메일로 받던 문의에 첨부파일과 처리 상태를 붙이려 합니다. 거래처가 자기 문의만 볼지, 같은 회사 문의도 볼지 정한 뒤 조회·등록·답변의 완성 조건을 씁니다. 첫 출시 범위를 한 세션에서 정할 수 있다면 기본 개발 흐름을 사용합니다.
- **현장 점검 결과 입력 도구:** 종이 점검표를 모바일로 바꿉니다. 첫 출시는 온라인 입력과 사진 첨부로 한정할지, 통신이 끊겨도 저장해야 할지 질문합니다. 오프라인 동기화까지 커지면 실험이나 별도 결정 관리가 필요한지 그때 판단합니다.
