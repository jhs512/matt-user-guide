...
| 용어 | 뜻 | 이 문서에서의 예 |
| --- | --- | --- |
| 스킬 | 에이전트가 특정 일을 수행할 때 따르는 작업 안내서. 지침과 보조 자료·스크립트를 포함할 수 있습니다. | `/grill-with-docs`는 질문으로 요구사항을 구체화하면서 용어와 중요한 결정을 기록합니다. |
| 에이전트 | 요청을 받아 필요한 자료를 읽고 도구를 사용해 작업하는 AI입니다. | 코드를 확인하고 수정한 뒤 테스트 결과를 보고합니다. |
| 서브 에이전트 | 주 에이전트가 범위를 나눠 맡긴 별도 에이전트입니다. | 주 에이전트가 요구사항을 정리하는 동안 특정 모듈의 현재 동작을 조사합니다. 자동 생성 여부는 스킬과 실행 환경에 따라 다릅니다. |
| 세션 | 에이전트와 작업을 이어가는 대화·실행 단위입니다. 이 가이드에서는 하나의 작업 맥락을 유지하며 논의하는 구간을 뜻합니다. | 오늘 소유권 정책을 논의하고, 다음 세션에서 결정 기록을 읽어 과금 정책을 이어갑니다. 메시지 한 번과 같은 뜻은 아닙니다. |
| 토큰 | 모델이 글·코드를 처리할 때 나누는 작은 단위입니다. 글자 수나 단어 수와 정확히 일치하지 않습니다. | 긴 코드와 대화를 많이 읽으면 처리해야 할 토큰도 늘어납니다. ‘토큰 1개 = 한글 1자’로 계산하지 않습니다. |
| 컨텍스트 | 모델이 지금 응답할 때 참고하는 지침·대화·파일 내용·도구 결과입니다. | 앞서 합의한 정책과 방금 읽은 코드가 포함됩니다. 저장소 전체가 항상 들어 있는 것은 아닙니다. |
| 컨텍스트 용량 | 모델이 한 번에 다룰 수 있는 컨텍스트의 한도입니다. | 논의가 길면 기록을 남기고 필요한 내용을 다시 읽어 이어갑니다. |
| 결정 지도 | 큰 작업에서 무엇을 결정해야 하고 무엇부터 풀어야 하는지 관리하는 지도입니다. Wayfinder에서는 지도 이슈와 자식 결정 티켓으로 유지합니다. | ‘자료 소유권’을 정한 뒤 ‘퇴사 시 접근 권한’을 정합니다. 시스템 구성도와는 다릅니다. |
| 결정 티켓 | 답을 얻거나 선택을 합의해야 끝나는 질문 단위입니다. | ‘기존 개인 프로젝트를 어느 조직에 귀속할 것인가?’ |
| 구현 티켓 | 합의된 동작을 만들고 검증하기 위해 나눈 작업 단위입니다. | ‘조직 초대 수락 기능과 권한 검증 구현’. 결정 티켓을 그대로 구현 목록으로 쓰지 않습니다. |
| 이슈 트래커 | 질문·작업·진행 상태·의존 관계를 저장하고 찾는 곳입니다. | 프로젝트 설정에 따라 GitHub 이슈나 로컬 Markdown 파일을 사용합니다. |
| 기획문서 | 제품의 목적, 사용자, 화면, 업무 규칙 등을 설명한 입력 자료입니다. 구현에 필요한 조건이 모두 확정돼 있지는 않을 수 있습니다. | ‘환불 관리 화면’ 기획서에 부분 환불이나 승인 권한이 빠져 있을 수 있습니다. |
| 명세 | 무엇을 만들고 어떤 동작을 만족해야 하는지 합의해 정리한 문서입니다. | 허용 입력, 실패 시 처리, 권한, 검증 조건을 포함합니다. |
| CONTEXT.md / ADR | 전자는 업무 용어집, 후자는 중요한 설계 선택의 이유를 기록한 문서입니다. | ‘조직’의 뜻은 용어집에, 조직별 과금을 선택한 이유는 필요할 때 ADR에 남깁니다. |
| 하네스 | 모델이 파일을 읽고 도구를 실행하며 일할 수 있게 해 주는 실행 환경입니다. | 스킬은 작업 안내서이고 하네스는 그 안내를 수행하는 환경입니다. |
| 워크플로우 | 상황에 맞춰 작업과 스킬을 이어 쓰는 흐름입니다. | 기획문서 읽기 → 빠진 조건 합의 → 명세 → 티켓 → 구현·검증. |
...
각 항목의 **실무 사례 3개**는 스킬을 고르는 기준을 설명하기 위한 가상 상황입니다. 실제 고객 사례나 이 저장소에서 확인한 구현 결과가 아닙니다. 특히 9번은 기존 서비스의 큰 변경을 상세히 다룹니다. 범위가 작으면 `/grill-with-docs`부터 시작해도 되며, 모든 작업에 Wayfinder를 끼울 필요는 없습니다.
...
| 9 | 큰 변경의 미결정 사항을 여러 세션에서 관리해야 한다 | 결정 티켓·의존 관계·합의된 목적지까지의 결정 기록 |
...
- **종이 점검표를 앱으로 바꾸는 기획서:** 필수 사진 3장과 제출 버튼은 있는데 현장 통신이 끊겼을 때 설명이 없습니다. 임시 저장, 사진 업로드 실패, 같은 점검의 중복 제출, 제출 후 수정 가능 여부를 질문합니다. 오프라인 동기화까지 필요하면 작은 실험으로 검토하고, 여러 세션의 결정이 필요할 만큼 커지면 Wayfinder로 관리합니다.
...
### 9. 운영 중인 서비스의 큰 변경을 여러 세션에서 결정해야 할 때

**상황:** 기존 서비스에 조직 계정을 도입하려는데 권한·결제·데이터 이전이 얽혀 있습니다. 한 영역을 결정하면 다른 영역에 새로운 질문이 생겨, 한 세션에서 전체 결정을 끝내기 어렵습니다.

**처음에 `/grill-with-docs`로 시작해도 됩니다.** 코드베이스가 크다고 Wayfinder를 먼저 실행해야 하는 것은 아닙니다. 작은 변경이면 인터뷰와 기록만으로 충분합니다. 신규 프로젝트도 같은 이유로 결정이 커졌을 때 Wayfinder를 사용할 수 있습니다.

| 비교 | `/grill-with-docs` | `/wayfinder` |
| --- | --- | --- |
| 공통점 | `grilling`과 `domain-modeling`으로 질문하고 용어·결정을 정리 | 지도 작성과 결정 논의에서 같은 스킬을 사용 |
| 질문의 선후관계 | `grilling` 자체가 선행 결정이 끝난 질문부터 진행 | 결정 티켓 사이의 의존 관계를 트래커에도 유지 |
| 주된 쓰임 | 계획·설계를 인터뷰로 구체화하면서 기록 | 한 세션을 넘는 큰 작업의 미결정 사항과 진행 상태를 공유·재개 |
| 기록의 역할 | `CONTEXT.md`는 용어집, ADR은 필요한 중요한 결정의 이유 | 지도 이슈는 색인, 개별 결정의 상세 답은 자식 티켓에 기록 |

**순서 A — 크기를 아직 모를 때:** `/grill-with-docs` → 여러 세션에 걸친 결정 관리가 필요해지면 `/wayfinder`로 기존 합의와 남은 질문 정리 → 지도 생성 → 후속 세션마다 결정 티켓 해결.

**순서 B — 처음부터 큰 작업임을 알 때:** `/wayfinder` → 목적지와 범위를 질문으로 합의 → 지도 생성 → 후속 세션마다 결정 티켓 해결.

두 경로 모두 필요한 결정이 정리된 뒤, 목적지가 구현 명세라면 `/to-spec` → `/to-tickets` → `/implement`로 이어갑니다. 지도만 작성하고 곧바로 명세로 넘어가는 흐름이 아닙니다. 트래커 설정이 없는 프로젝트는 `/setup-matt-pocock-skills`로 준비합니다. 이미 준비되어 있다면 다시 설정할 필요는 없습니다.

#### 실무 사례 1 — 개인용 SaaS에 기업 조직 계정 도입

**가정한 현장:** 프로젝트·첨부파일·구독이 모두 `user_id`에 묶인 서비스입니다. 기업 고객이 “직원이 퇴사해도 회사 자료와 구독은 남아야 한다”고 요청합니다. 프로젝트 소유자는 한 명인데, 한 사용자가 두 회사에서 일하는 사례도 있습니다.

**그릴 위드 독스로 시작할 질문:** “사용자와 조직 중 누가 프로젝트를 소유하는가?”, “퇴사자는 자료 접근만 잃는가, 개인 자료도 이전하는가?”를 기존 코드와 용어에 맞춰 논의합니다. 조직 초대 화면만 추가하고 소유권은 그대로라면 이 정도 논의로 끝날 수도 있습니다.

**Wayfinder로 옮길 시점:** 소유권 결정에 따라 과금 단위, API 권한 검사, 개인 프로젝트 이전 정책까지 각각 조사와 합의가 필요해졌을 때입니다. 단지 테이블이 많다는 이유로 옮기지 않습니다.

**결정 지도 예시:** ‘조직과 개인의 자료 소유권’ → ‘퇴사·초대 취소 시 접근 정책’, ‘조직별 과금 주체’. 소유권과 기존 데이터 조사 결과가 갖춰지면 ‘기존 프로젝트의 조직 배정 기준’을 결정합니다. 예를 들어 공동 사용 중인 개인 프로젝트가 발견되면 ‘소유 조직을 누가 확정하는가’라는 새 질문을 추가합니다.

**첫 세션과 다음 세션:** 첫 세션은 ‘조직 계정 전환 명세에 필요한 결정을 마친다’는 목적지와 질문 지도를 만듭니다. 다음 세션은 선행 조건이 해결된 ‘조직과 개인의 자료 소유권’ 티켓 하나를 사람과 논의합니다. ‘모든 테이블에 organization_id 추가’는 이 단계의 결정 티켓이 아니라, 설계가 정해진 뒤 검토할 구현 작업입니다.

```text
/wayfinder
개인용 SaaS에 기업 조직 계정을 도입하려고 해.
현재 user_id에 묶인 프로젝트·파일·구독의 코드와 기존 ADR을 확인해줘.
이미 합의한 내용은 이어받고, 아직 모르는 사실은 확인된 사실과 구분해줘.
목적지는 조직 전환 명세를 쓸 수 있도록 소유권·권한·과금·이전 정책을 결정하는 것이야.
먼저 목적지와 범위를 나와 확인하고 결정 지도와 선후관계를 만들어줘.
```

#### 실무 사례 2 — 쇼핑몰의 결제·환불을 새 결제사로 이전

**가정한 현장:** 기존 결제사로 승인받은 주문이 남아 있고, 한 주문의 여러 상품을 나눠 취소할 수 있습니다. 새 결제사를 붙이되 이전 주문의 환불과 정산도 계속 처리해야 합니다. 콜백이 늦거나 중복 도착하는 상황도 검토해야 합니다.

**그릴 위드 독스로 충분한 범위:** 기존 연동의 오류 문구를 바꾸거나, 이미 합의된 정책대로 결제 실패 안내를 추가하는 일입니다. 결제 코드가 크다는 이유만으로 지도가 필요하지 않습니다.

**Wayfinder가 필요한 범위:** ‘이전 주문을 어느 결제사에서 환불할 수 있는가’는 계약·공식 API 문서 조사가 필요하고, 그 결과에 따라 ‘주문별 결제사 선택 기준’, ‘부분 취소 상태 모델’, ‘전환 중 새 결제 차단과 복구 기준’을 결정해야 하는 경우입니다.

**결정 지도 예시:** ‘기존 승인 건의 취소 지원 조건 조사’ → ‘이전 주문의 환불 경로’. ‘콜백 중복·순서 역전 시 상태 규칙’을 합의한 뒤 작은 상태 모델 실험으로 검토합니다. 두 경로가 정리되면 ‘신규 주문 전환 조건과 실패 시 복구 범위’를 결정합니다. 외부 계약·API 동작은 이 예시가 확정해 주는 사실이 아닙니다.

**완료 기준:** 이전 주문과 신규 주문의 결제·취소 경로, 중복 처리 기준, 전환·복구 조건이 결정되어 구현 명세에 넣을 수 있습니다. 실제 결제나 환불을 실행하거나 운영 전환을 마쳤다는 뜻은 아닙니다.

```text
/wayfinder
기존 결제사의 미취소 주문을 유지하면서 신규 주문부터 새 결제사로 보내려 해.
목적지는 결제·부분 취소·전환·복구 정책을 결정해 이전 명세를 작성할 준비를 마치는 것이야.
현재 주문 상태와 연동 코드를 확인하고, 계약·API 확인이 필요한 질문을 분리해줘.
결정 티켓의 의존 관계를 만들고 실제 결제·환불·운영 전환은 이 계획 범위에 넣지 마.
```

#### 실무 사례 3 — 모놀리스에서 재고 기능을 분리

**가정한 현장:** 주문 생성과 재고 차감이 같은 DB 트랜잭션 안에 있습니다. 행사 때 주문 부하가 올라 재고 기능을 분리하자는 제안이 나왔지만, 주문 실패·결제 시간 초과·취소 시 예약 수량을 돌려주는 규칙이 코드 여러 곳에 흩어져 있습니다.

**그릴 위드 독스로 시작할 질문:** “실제 병목이 재고 처리인가?”, “분리의 목적이 처리량인가, 별도 배포인가?”부터 확인합니다. 조사 결과 특정 조회나 모듈 내부 정리로 해결된다면 그 범위로 진행합니다. 서비스 분리를 처음부터 정답으로 두지 않습니다.

**Wayfinder로 옮길 시점:** 분리 여부를 판단하려면 부하 근거, 재고 예약·확정·해제 모델, 이벤트 전달 실패, 기존 예약 이전, 전환 중 데이터 책임을 여러 세션에서 따져야 할 때입니다.

**결정 지도 예시:** ‘현재 병목과 트랜잭션 범위 확인’ → ‘분리가 필요한가’. 분리가 필요하다고 결정되면 ‘예약·확정·해제의 소유 주체’ → ‘주문 실패·메시지 재전달 시 정합성 규칙’ → ‘기존 예약 데이터 이전 및 전환 조건’으로 질문을 구체화합니다. 질문 자체가 아직 불명확한 영역은 억지로 티켓을 만들지 않고 지도에 미정 영역으로 남깁니다.

**완료 기준:** 분리 여부와 그 이유, 분리한다면 유지해야 할 동작과 실패 처리·전환 조건이 합의됩니다. 목적지가 ‘분리 여부 결정’이었다면 그 결정에서 지도가 끝나며, 서비스 구현까지 자동 확장하지 않습니다.

```text
/wayfinder
주문과 재고가 같은 DB 트랜잭션에 있는 모놀리스에서 재고 분리를 검토하고 있어.
아직 분리하기로 확정하지 않았어. 목적지는 분리 여부와 판단 근거를 결정하는 것이야.
현재 코드·관측 가능한 성능 자료를 확인하고 결정에 필요한 질문 지도를 만들어줘.
분리 불필요라는 결론도 허용하고, 구현 티켓은 아직 만들지 마.
```

**지도를 이어갈 때:** 생성된 지도 식별자를 사용해 `/wayfinder [실제 지도 URL 또는 번호]`로 재개합니다. 별도 티켓을 지정하지 않으면 진행 가능한 미할당 결정 티켓을 선택합니다. 원문은 연구 티켓을 제외하고 세션당 하나만 해결하도록 하며, 사람의 판단이 필요한 질문은 실제 답변을 받아 해결합니다. 답을 티켓에 남기고 닫은 다음, 지도에는 요약과 링크를 추가하고 새로 드러난 질문을 연결합니다.

**끝나면:** 합의한 목적지까지 필요한 결정이 정리되고, 후속 담당자가 결정 근거와 남은 작업을 찾을 수 있습니다. 기본은 계획이며, 실행까지 포함하려면 지도 Notes에 별도로 범위를 정해야 합니다.
...
2026-09-06에 로컬에 설치된 각 `SKILL.md`를 읽고 다음을 확인했습니다. 아래 경로는 스킬 저장소 안의 상대 경로입니다. 최신 원격 원문은 조회에 실패하여 최신 버전과의 일치까지 확인한 것은 아닙니다.
...
| `wayfinder/SKILL.md`의 Map·Invocation | 여러 세션에 걸친 결정 지도와 자식 티켓을 트래커에 유지합니다. 지도 작성과 티켓 해결은 구분하며 연구를 제외하면 세션당 하나의 티켓을 해결합니다. |
| `wayfinder/SKILL.md`의 Plan, don't do·Ticket Types | 기본은 계획입니다. 사람과 논의할 결정은 AI가 대신 답하지 않습니다. 조사·시제품·결정을 가능하게 하는 준비 작업을 구분합니다. |
...
‘그릴 위드 독스로 시작했다가 커지면 Wayfinder로 전환’은 두 스킬의 역할에 근거한 **이 가이드의 사용 제안**이며, 원문이 강제하는 고정 순서는 아닙니다. 위 실무 사례의 데이터 모델·정책·서비스 제약은 실제 프로젝트에서 따로 확인해야 합니다.
