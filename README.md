...
- **메인 에이전트(주 에이전트)** == 코딩 에이전트 프로그램 내부의 대화창 마다 존재하는 실질적인 AI
- **서브 에이전트** == 메인 에이전트가 생성하고 일을 지시하는 보조 AI
- **세션** == 에이전트가 이어가는 대화의 흐름. 각 에이전트는 본인만의 세션을 가진다.
- **스킬** == 에이전트가 특정 작업을 할 때 따르는 안내서
- **하네스** == 에이전트가 파일을 읽고 도구를 실행할 수 있게 해 주는 환경 + 안전작업지침
...
- **트리아지(triage)** == 외부에서 들어오는 요구사항을 티켓화 해주는 작업
