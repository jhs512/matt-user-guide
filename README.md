...
### /implement도 서브에이전트에 맡길까요?

**명세와 완료 조건이 분명하고 구현이 오래 걸린다면 좋은 선택입니다.** 주 에이전트는 사용자와의 논의·전체 진행·최종 확인을 맡고, 서브에이전트는 별도 worktree에서 구현·테스트·리뷰를 수행하게 할 수 있습니다. 작은 수정은 주 에이전트가 바로 처리하는 편이 간단합니다. 위임에는 추가 비용과 결과를 합치는 작업이 따릅니다. [Codex 서브에이전트 안내](https://learn.chatgpt.com/docs/agent-configuration/subagents)

작업 방식을 처음 정할 때 짧게 요청합니다.

```text
이번 구현은 서브에이전트가 별도 worktree에서 /implement로 진행해줘.
결과 확인과 변경 통합, 작업용 서버·worktree 정리까지 맡아줘.
```

이후 같은 범위와 방식으로 이어갈 때는 `/implement`만 호출하면 됩니다. **스킬 이름만으로 서브에이전트 실행이나 worktree 생성이 자동 보장되는 것은 아닙니다.** 주 에이전트가 합의한 방식을 적용해야 합니다.

**순서:** 명세·티켓 전달 → 별도 worktree에서 구현 → 테스트·리뷰·커밋 → 주 에이전트가 결과 확인 → 합의한 대상 브랜치에 변경 통합 → 통합 결과 검증 → 작업용 서버 종료·worktree 정리

서로 의존하는 티켓은 앞 작업의 결과를 반영한 뒤 진행합니다. 여러 서브에이전트로 병렬 구현하는 것은 독립적인 작업에 적합합니다. worktree가 달라도 포트·외부 DB 같은 실행 자원은 공유될 수 있으므로 실행 환경도 구분합니다.

서브에이전트의 완료 보고만으로 전체 작업이 끝난 것은 아닙니다. 주 에이전트가 커밋과 검증 결과를 확인하고, 통합 후 필요한 검사를 마쳐야 합니다. 결과가 아직 통합되지 않았거나 커밋되지 않은 파일이 있으면 먼저 보존합니다. 작업용 서버를 종료한 뒤 worktree를 제거하고, 브랜치는 팀의 보관 규칙에 따라 처리합니다. 운영 서버는 이 뒷정리 대상이 아닙니다.
