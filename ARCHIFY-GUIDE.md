...
[Matt 가이드의 10개 워크플로우](README.md#그외-워크플로우-10개)를 실제 코드 이해와 연결하는 예시입니다. **각 항목의 세 사례는 가상 실무 상황**이며 이 저장소에서 확인한 시스템이 아닙니다. Archify는 현재 구조나 제안 구조를 그리는 도구입니다. 질문에 대한 합의, 버그 수정, 배포 완료는 각 작업에서 따로 확인합니다. 그림이 판단에 도움이 될 때만 추가하세요.

### 1. 새 업무 도구를 처음 만들 때

**흐름:** `/grill-with-docs`로 범위 합의 → 필요하면 `/archify`로 제안 흐름 확인 → 명세·티켓·구현.

- **장비 대여:** 신청 → 승인 → 지급 → 반납 흐름을 그리고, 같은 노트북의 중복 배정을 누가 막는지 논의합니다. 코드가 없으면 제안으로 표시합니다.
- **거래처 문의 포털:** 거래처 담당자와 사내 담당자의 조회·답변 범위를 그립니다. 같은 거래처 직원 사이의 문의 공유는 사람이 정할 정책입니다.
- **현장 점검 앱:** 사진 첨부와 점검 결과 제출 흐름을 그립니다. 오프라인 저장을 구현하지 않았다면 점선 제안으로 구분하고 실제 동작처럼 표시하지 않습니다.

**판단:** 처음 만드는 앱도 `/grill-with-docs`로 시작할 수 있습니다. 빈 저장소라는 이유로 Wayfinder가 필요한 것은 아닙니다.
